...
class JsonParser
{
	...
	public static function parse(array $jsonArray, IReflectedObject $object, $prefix = '')
	{
		...
		foreach ($jsonArray as $key => $value) {
			...
			if (is_scalar($value)) {
				$ref->setPropertyValue($propName, new ValueScalar($value));
				continue;
			}
			...
			if (is_array($value)) {
				if ($propType->type == TypeName::Array) {
					if (is_scalar($value[0])) {
						...
						$ref->setPropertyValue($propName, new ValueScalars($vector));
					} else {
					...
					$ref->setPropertyValue($propName, new ValueObject($propInstance));
				}
			}
		}
	}
