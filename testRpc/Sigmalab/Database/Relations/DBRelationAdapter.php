...
abstract class DBRelationAdapter
{
	...
	/**
	 * @param \Sigmalab\Database\Core\IDataSource $connection
	...
	function select($objectID, $order = "")
	{
		if ( is_object( $objectID)) {
			throw new DatabaseException("Expected number but object given as objectID. Got: ".get_class( $objectID));
		}
		if ( !$objectID ) {
			return array();
		}
		...
		$stm = $this->stmSelectChildren( $objectID, $order );
		return $this->database()->execute($stm);
	}
	 ...
	 */
	function stmSelectChildren($parentIds, $order=null )
	{
